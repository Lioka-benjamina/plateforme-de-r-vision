            ...
            <div className="lg:pr-16 xl:pr-24">
              <div className="flex items-center gap-2 mb-8">
                <div className="flex -space-x-2">
                  {['V', 'M', 'A', 'L'].map((l, i) => (
                    <div key={i} className="w-8 h-8 rounded-full border-2 border-surface-900 bg-primary-600 flex items-center justify-center text-[10px] font-bold text-white">
                      {l}
                    </div>
                  ))}
                </div>
              ...
              <h1 className="text-4xl sm:text-5xl xl:text-6xl font-black leading-[1.05] tracking-tight mb-6 text-white">
                La plateforme<br />
                qui{' '}
              ...
              <div className="flex flex-col sm:flex-row gap-4">
                <Link
                  to="/register"
                  className="inline-flex items-center justify-center gap-2 bg-primary-600 hover:bg-primary-500 text-white px-8 py-4 rounded-2xl font-bold transition-all active:scale-[0.98] shadow-lg shadow-primary-900/40"
                >
                ...
                <Link
                  to="/catalog"
                  className="inline-flex items-center justify-center gap-2 bg-white/5 hover:bg-white/10 border border-white/10 text-white px-8 py-4 rounded-2xl font-semibold transition-all text-sm"
                >
                ...
                {stats.map((s, i) => (
                  <div
                    ...
                    className={`rounded-3xl p-7 border ${
                      i === 1
                        ? 'bg-primary-600 border-primary-500 col-span-1 shadow-lg shadow-primary-600/20'
                  ...
                  >
                    ...
                    <p className={`text-sm font-semibold mt-1 ${i === 1 ? 'text-primary-100' : 'text-surface-700'}`}>
                      {s.label}
                    </p>
        ...
        <div className="container mx-auto px-4 sm:px-6 lg:px-8">
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
            {features.map((f, i) => {
              const Icon = f.icon
              return (
                <div key={f.title} className={`relative rounded-3xl overflow-hidden ${i === 1 ? 'md:-translate-y-4' : ''}`}>
                  <div className={`h-1.5 bg-gradient-to-r ${f.gradient}`} />
                  <div className="bg-white border border-surface-100 border-t-0 rounded-b-3xl p-8">
                    <span className={`inline-block text-[11px] font-bold uppercase tracking-widest px-3 py-1.5 rounded-lg mb-5 ${f.soft}`}>
                      {f.tag}
                    </span>
                    <div className={`w-12 h-12 rounded-2xl flex items-center justify-center mb-5 bg-gradient-to-br ${f.gradient}`}>
              ...
              return (
                <div
                  ...
                  className={`rounded-3xl p-8 transition-all duration-300 ${
                    r.featured
                      ? `bg-surface-950 border ${r.border} shadow-lg`
                      : `bg-white ${r.border} border hover:shadow-card-hover hover:-translate-y-1`
                  }`}
                >
                  <div className={`w-12 h-12 rounded-2xl flex items-center justify-center mb-6 ${r.featured ? 'bg-primary-600' : r.bg}`}>
                    <Icon size={22} className={r.featured ? 'text-white' : r.color} />
                  </div>
                  <h3 className={`text-xl font-bold mb-2 ${r.featured ? 'text-primary-400' : 'text-surface-900'}`}>
                    {r.role}
                  </h3>
                  ...
                  <ul className="space-y-3">
                    {r.perks.map((perk) => (
                      <li key={perk} className="flex items-center gap-3 text-sm">
                        <CheckCircle2 className={`w-4 h-4 flex-shrink-0 ${r.featured ? 'text-primary-400' : 'text-success-500'}`} />
        ...
        <div className="container mx-auto px-4 sm:px-6 lg:px-8">
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-12 items-center max-w-5xl mx-auto">
            <div>
              ...
              <h2 className="mt-3 text-3xl md:text-4xl font-black text-white tracking-tight leading-tight">
            ...
            <div className="flex flex-col sm:flex-row lg:flex-col xl:flex-row gap-4 lg:items-start xl:items-center">
              <Link
                to="/register"
                className="inline-flex items-center justify-center gap-2 bg-primary-600 hover:bg-primary-500 text-white px-8 py-4 rounded-2xl font-bold transition-all active:scale-[0.98] text-base shadow-xl shadow-primary-900/50"
              >
              ...
              <Link
                to="/catalog"
                className="inline-flex items-center justify-center gap-2 border border-surface-700 hover:border-surface-500 text-surface-300 hover:text-white px-8 py-4 rounded-2xl font-semibold transition-all text-sm"
              >
