...
const inputClass = "w-full border border-surface-200 rounded-lg px-4 py-2.5 text-sm text-surface-900 placeholder:text-surface-400 focus:outline-none focus:ring-2 focus:ring-primary-500/30 focus:border-primary-400 transition-all bg-surface-50"
...
export default function ContactPage() {
  ...
  return (
    <div className="max-w-4xl mx-auto px-4 sm:px-6 py-8 space-y-8">
      {/* Hero */}
      <section className="rounded-xl bg-primary-600 p-10 md:p-14 text-center">
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
        {infoCards.map(({ icon: Icon, label, value }) => (
          <div key={label} className="bg-white rounded-xl border border-surface-100 p-5 text-center hover:shadow-sm transition-shadow">
            <div className="w-10 h-10 bg-primary-50 rounded-lg flex items-center justify-center mx-auto mb-3">
              <Icon className="w-5 h-5 text-primary-600" />
            </div>
            <h3 className="font-semibold text-surface-900 text-sm">{label}</h3>
      ...
      {/* Form */}
      <div className="bg-white rounded-xl border border-surface-100 p-6 md:p-8">
        <h2 className="text-lg font-semibold text-surface-900 mb-6">Envoyez-nous un message</h2>

        {success && (
          <div className="flex items-start gap-3 bg-emerald-50 border border-emerald-200 text-emerald-700 px-4 py-3.5 rounded-xl text-sm mb-6">
            <CheckCircle2 className="w-4 h-4 mt-0.5 flex-shrink-0" />
            Message envoyé avec succès ! Nous vous répondrons dans les plus brefs délais.
          </div>
        )}
        {error && (
          <div className="bg-rose-50 border border-rose-200 text-rose-700 px-4 py-3.5 rounded-xl text-sm mb-6">
            {error}
          </div>
        )}
        ...
        <form onSubmit={handleSubmit} className="space-y-5">
          ...
          <button
            type="submit"
            disabled={loading}
            className="inline-flex items-center gap-2 bg-primary-600 hover:bg-primary-700 disabled:opacity-60 text-white text-sm font-semibold px-6 py-2.5 rounded-lg transition-colors"
          >
